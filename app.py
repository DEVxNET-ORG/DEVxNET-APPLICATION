...
@login_required
def dashboard():
    user_id = session.get('id')  # Fetch the logged-in user's ID from session

    if not user_id:
        flash('User ID not found in session, please log in again.', 'danger')
        return redirect(url_for('employee_login'))

    # Query to get the username for the logged-in user based on the user ID
    cur = mysql.connection.cursor()
    cur.execute("SELECT username FROM employee WHERE id = %s", (user_id,))
